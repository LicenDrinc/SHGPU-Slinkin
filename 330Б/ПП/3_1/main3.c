...
void crf(char* f) { FILE* file = fopen(f, "w"); if (file) { fclose(file); } }
void ch(char* f, struct stat s) { if (chown(f, s.st_uid, s.st_gid) < 0) { printf("Ошибка изменения владельца %i, %i, в %s!\n", s.st_uid, s.st_gid, f); } }
...
int MissionCopy(char* src, char* dest, int line, int lock)
{
    ...
    struct stat filedata; int action = -1; int filetype = 0; char buf[4096];
    int s = lstat(src, &filedata); if (s == 0) filetype = filedata.st_mode / 4096;
    switch (filetype)
    {
		case DT_UNKNOWN: case DT_FIFO: case DT_CHR: case DT_BLK: case DT_SOCK: case DT_WHT: action = 0; break;
		case DT_DIR: action = 1; break;
		case DT_REG: case DT_LNK: action = 2; break;
    }
    if (s == 0)
    {
        if (action == 2)
        {
            ...
            {
				int readamount = 1; FILE* source = fopen(src, "r"); FILE* target = fopen(dest, "w");
				while (readamount > 0) { readamount = fread(buf, 1, 4096, source); fwrite(buf, readamount, 1, target); }
				fclose(target); fclose(source);
			}
        }
        else if (action == 1)
        {
            ...
            if (!basedir)
            {
				printf("Ошибка: Нельзя открыть: %i: %s\n", errno, strerror(errno)); errno = 0; printf("\n"); closedir(basedir);
				chmod(dest, filedata.st_mode % (int)powf(8, 5)); ch(dest, filedata); return 1;
			}
            if (access(src, 1)) { printf("Предуприждение: Нельзя открыть!\n"); }
        ...
        else if (action == 0) { crf(dest); }
		chmod(dest, filedata.st_mode % (int)powf(8, 5)); ch(dest, filedata);
    }
