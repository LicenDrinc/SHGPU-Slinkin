...
int ChekResurs(char* filename, ServerStat* stats)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (idx == -1) { strcpy(stats[count].name, server); stats[count].count = 1; count++; }
        else stats[idx].count++;
    }
...
{
    int fd = open(rf, O_RDWR | O_CREAT, 0777); if (fd < 0) { perror("open result"); exit(1); }

    flock(fd, LOCK_EX); FILE* f = fdopen(fd, "r+");
    ServerStat inFile[MAX_SERVERS]; int inFileCount = 0; char name[MAX_NAME]; int cnt;

    while (fscanf(f, "%2559s %d", name, &cnt) == 2)
    { strcpy(inFile[inFileCount].name, name); inFile[inFileCount].count = cnt; inFileCount++; }
    ...
    for (int i = 0; i < localCount; i++)
    {
        ...
        if (idx == -1)
        {
			strcpy(inFile[inFileCount].name, localStats[i].name);
			inFile[inFileCount].count = localStats[i].count; inFileCount++;
		}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 2; i < argc; i++)
    {
        ...
        if (pid == 0)
        {
			ServerStat stats[MAX_SERVERS]; int count = ChekResurs(argv[i], stats);
			PrintResult(result_file, stats, count); exit(0);
		}
    }
