...
uses
  Classes, SysUtils, unit1, DateUtils;
...
begin
    SetLength(PrItems, Length(PrItems) + 1);
    PrItems[High(PrItems)] := Person;
end;
...
begin
      ...
      PrItems[Index].Free;
      for I := Index to High(PrItems) - 1 do
          PrItems[I] := PrItems[I + 1];
      SetLength(PrItems, Length(PrItems) - 1);
end;
...
begin
    ...
    try
        ...
        for I := 0 to High(PrItems) do
        begin
          WriteLn(F, PrItems[I].FullName);

          if PrItems[I].Gender then
              GenderStr := 'Male'
          else
              GenderStr := 'Female';
          WriteLn(F, GenderStr);

          WriteLn(F, DateToStr(PrItems[I].BirthDate));
          WriteLn(F, PrItems[I].ID);

          if PrItems[I].Father <> nil then
              WriteLn(F, PrItems[I].Father.ID)
          else
              WriteLn(F, '');

          if PrItems[I].Mother <> nil then
              WriteLn(F, PrItems[I].Mother.ID)
          else
              WriteLn(F, '');

          if I < High(PrItems) then
              WriteLn(F);
        end;
    finally
        CloseFile(F);
...
begin
    ...
    for I := 0 to High(PrItems) do
      for J := I + 1 to High(PrItems) do
        if PrItems[I].ID = PrItems[J].ID then
          raise EDublicateID.CreateFmt('Duplicate ID found: %s', [PrItems[I].ID]);

    // Проверка родителей
    for I := 0 to High(PrItems) do
    begin
        // Проверка пола родителей
        if (PrItems[I].Father <> nil) and (PrItems[I].Mother <> nil) then
            if PrItems[I].Father.Gender = PrItems[I].Mother.Gender then
                raise ESameGenderParents.CreateFmt('Parents have same gender for person: %s', [PrItems[I].ID]);

        // Проверка возраста родителей
        if PrItems[I].Father <> nil then
        begin
            Parent := PrItems[I].Father;
        ...
        begin
            SetLength(Visited, 0);
            if HasCycle(PrItems[I], PrItems[I].Father) then
                raise EParentCycle.CreateFmt('Cycle detected in father lineage for person: %s', [PrItems[I].ID]);
        end;

        if PrItems[I].Mother <> nil then
        begin
            SetLength(Visited, 0);
            if HasCycle(PrItems[I], PrItems[I].Mother) then
                raise EParentCycle.CreateFmt('Cycle detected in mother lineage for person: %s', [PrItems[I].ID]);
        end;
    end;
end;
