...
procedure sumTest(currTest:integer; var oneTest:TOneTest; var tests:TAllTests);
begin
	tests[currTest].midtest := oneTest;
end;
}
...
begin
	...
	begin
		{
		flag := true;
		for i1 := 0 to Length(altTests) - 1 do
		begin
			if (altTests[i1].name = tests[i].name) then
			begin
				flag := false;
				break;
			end;
		...
		if (flag) then
		begin
			setLength(altTests,Length(altTests) + 1);
			altTests[Length(altTests)-1].name := tests[i].name;
			altTests[Length(altTests)-1].count := 0;
			for i1 := 1 to 20 do
			begin
				for i2 := 0 to 4 do
				begin
					altTests[Length(altTests)-1].midtest[i1][i2] := 0;
				end;
			end;
		end;
		}
	...
	begin
		...
		begin
			if (altTests[i].name = tests[i1].name) then
			begin
				altTests[i].count := altTests[i].count + 1;
				for i2 := 1 to 20 do
				begin
					for i3 := 0 to 4 do
					begin
						if (tests[i1].midtest[i2][i3] <> -1) then
						begin
							oneTest[i2][i3] := oneTest[i2][i3] + tests[i1].midtest[i2][i3];
						end;
					end;
				end;
			end;
		...
		begin
			for i2 := 1 to 4 do
			begin
				if (altTests[i].count = oneTest[i1][0]) then
				begin
					oneTest[i1][i2] := -1;
				end
				else
				begin
					oneTest[i1][i2] := oneTest[i1][i2] div (altTests[i].count - oneTest[i1][0]);
				end;
			end;
		end;
		altTests[i].midtest := oneTest;}
	end;
